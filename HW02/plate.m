...
%legend("\omega_+","\omega_-" , "Linewidth" , 12 );
hold on
plot( w_s.*eps, ones(length(eps)).*w_s , "LineStyle" , "--" , "color" , "k" )
plot(w_s.*eps, w_s.*(1 + 2.*eps), "LineStyle" , "--" , "color" , "k" );

xlabel("\omega_0 \times \epsilon [rad/s]" , "Linewidth" , 18);
ylabel("\omega [rad/s]" , "Linewidth" , 18 );

subplot(2,1,2)
plot(w_s.*eps, tau_1 , w_s.*eps, tau_2 ); %decay time
%legend(" \tau_+","\tau_-" , "Linewidth" , 12);
ylim([-2 2])
xlabel("\omega_0 \times \epsilon [rad/s]" , "Linewidth" , 18 );
ylabel("\tau [s]" , "Linewidth" , 18);