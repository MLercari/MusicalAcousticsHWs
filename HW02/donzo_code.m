...
nasa_coeffs_f = [8.6667 36.651 64.844 94.147 103.32];

w_nasa_f = zeros(1,5);

w_nasa_f = nasa_coeffs_f./(Lx^2*sqrt(ro_nasa/D));
...
w_nasa_c = nasa_coeffs_c./(1*sqrt(ro_nasa/D));
...
for i = 1:5
    subplot(5,1,i)
    plot(t, real(fp_f(i,1)));
end

figure (2)
for i = 1:5
    subplot(5,1,i)
    plot(t, real(fp_c(i,1)));
end
 
figure (3)
for i = 1:5
    subplot(5,1,i)
    plot(t, real(fp_s(i,1)));
end
