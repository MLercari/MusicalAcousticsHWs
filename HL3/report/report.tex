...
The string circuit is connected to the body circuit at the U2 switch (Fig. \ref{fig:guitarstring}. The sound is recovered by taking the time derivative (approximated with finite differences) of the current at this point, which can be shown to be proportional to the vertical force at the bridge.  The resulting signal can be seen in Fig. \ref{fig:complete} and it has been saved as an audio file in \texttt{completeGuitar.wav}.
