...
figure(3);
fplot(cot(y*L2) + cot(y*L1corr) + 1/(y*x1));
sols = zeros(1,4);
sols(1) = vpasolve(cot(y*L2) + cot(y*L1corr) + 1/(y*x1) == 0, y, 3);
sols(2) = vpasolve(cot(y*L2) + cot(y*L1corr) + 1/(y*x1) == 0, y, 6);
sols(3) = vpasolve(cot(y*L2) + cot(y*L1corr) + 1/(y*x1) == 0, y, 9.2);
sols(4) = vpasolve(cot(y*L2) + cot(y*L1corr) + 1/(y*x1) == 0, y, 12.1);
...
for i=1:length(f_max_2)
    xline(f_max_2(i), '--', compose('%.1f Hz', f_max_2(i)), 'lineWidth' , 1.1, 'color', '#980002');
    xline(f_min_2(i), '-.', compose('%.1f Hz', f_min_2(i)), 'lineWidth' , 1.1, 'color', '#5b7c99');
end
